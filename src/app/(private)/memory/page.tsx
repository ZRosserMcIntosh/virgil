    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-xs text-bone-400">
          Sensitive entries are encrypted at rest. Items marked “never cloud” are
      ...
      <div className="v-card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {memories.length === 0 && (
              <tr><td className="px-4 py-6 text-bone-400" colSpan={6}>No memory records yet.</td></tr>
            )}
            {memories.map((m) => (
              <tr key={m.id} className="border-t border-ink-700">
                <td className="px-4 py-3 text-bone-50">
                  {m.title}
                  {m.encrypted && <span className="ml-2 text-[10px] text-signal-amber">encrypted</span>}
                </td>
