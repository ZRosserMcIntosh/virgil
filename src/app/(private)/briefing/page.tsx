    ...
    <div className="v-card v-card-pad">
      ...
      {items.length === 0 ? (
        ...
        <ul className="mt-3 space-y-3">
          {items.map((it, i) => (
            <li key={i}>
              <div className="text-sm text-bone-50">{it.title}</div>
              <div className="text-xs text-bone-300">{it.detail}</div>
              <div className="mt-0.5 text-[10px] uppercase tracking-wider text-bone-400">
